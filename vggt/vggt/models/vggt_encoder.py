...
class VGGT_encoder(nn.Module, PyTorchModelHubMixin):
    ...
    def camera_tokens_agg(self,camera_tokens,token_type,relative_pose_enc = None):
        ...
        if token_type == "multiview":
            ...
            camera_tokens = relative_pose_enc + camera_tokens # 1 6 1 2048
        if token_type == "frame":
    ...
    def forward(self, images: torch.Tensor, query_points: torch.Tensor = None,others = None):
        ...
        if self.rope is not None:
            pos = self.position_getter(B * self.cam_num, H // self.patch_size, W // self.patch_size, device=images.device)
            pos = pos + 1
            pos_special = torch.zeros(B * self.cam_num, 2, 2).to(images.device).to(pos.dtype)
        ...
        for layer in range(layer_nums):
            ...
            aggregated_tokens = self.batch_norm(aggregated_tokens.reshape(-1,745,2048).permute(0,2,1)).permute(0,2,1).reshape(B,6,-1,745,2048)
            
            B,_,_,N,C = aggregated_tokens.size() # 1 6 9 745 2048
            
            frame_relative_tokens = relative_pose_enc.unsqueeze(2).unsqueeze(2) # 1 6 1 1 2048
            frame_relative_tokens = frame_relative_tokens.expand(B,6,f_num,1,2048) # 1 6 9 1 2048
            frame_pose_tokens = aggregated_tokens[:,:,:,0,:].unsqueeze(-2) # 1 6 9 1 2048
            last_frame_pose_enc = last_pose_enc.reshape(B,self.cam_num,f_num,1,C) # 1 6 9 1 2048
            frame_tokens = torch.cat([frame_relative_tokens,last_frame_pose_enc+frame_pose_tokens,aggregated_tokens[...,5:,:]],dim=-2) # 1 6 742 2048
    

            atten_idx = 0
            for _ in range(4):
                frame_tokens, atten_idx = self._process_mv_attention(frame_tokens,B,6,742,2048,atten_idx)
            agg_frame_tokens_list.append(frame_tokens.unsqueeze(2))

            
            agg_frame_tokens = torch.cat(agg_frame_tokens_list,dim=2).view(B,-1,742,2048) # 1 54 742 2048
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            if self.camera_head is not None:
                ...
                predictions["mv_enc_list"] = self.camera_relative_head(agg_layers_camera_relative_tokens_list,relative_pose_enc = relative_pose_enc)
        ...
        # assert 1==0
        return predictions
    def _process_mv_attention(self, tokens, B, S, P, C, mv_idx):
        """
        Process global attention blocks. We keep tokens in shape (B, S*P, C).
        """
        
        # tokens = tokens.view(B, S , P, C) # 1 54 742 2048
        ...
        if self.training:
            tokens = checkpoint(self.mv_blocks[mv_idx], tokens, use_reentrant=False)
        else:
            tokens = self.mv_blocks[mv_idx](tokens)
        mv_idx += 1
