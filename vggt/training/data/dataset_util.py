...
def crop_image_depth_and_intrinsic_by_pp(
    ...
    # print((target_shape[0] / 2), cx,(target_shape[1] / 2), cy)
    if strict:
        half_x = max((target_shape[0] / 2), cx)
        half_y = max((target_shape[1] / 2), cy)
    ...
    start_x = 0
    start_y = 0
    # print(start_x,start_y,image.shape,depth_map.shape)
    assert start_x >= 0
    assert start_y >= 0
