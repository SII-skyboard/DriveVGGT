...
class MultitaskLoss(torch.nn.Module):
    ...
    def forward(self, predictions, batch) -> torch.Tensor:
        ...
        if "pose_enc_list" or "mv_enc_list" in predictions:
            if self.camera["decoder"] == "raw_decoder":
            ...
            elif self.camera["decoder"] == "seperate_decoder":
                camera_loss_dict = compute_camera_decoder_loss(predictions, batch, **self.camera)
                camera_loss = camera_loss_dict["loss_camera"] * self.camera["weight"]
                total_loss = total_loss + camera_loss
                loss_dict.update(camera_loss_dict)
                
            # temporal_consistency loss
            if self.camera["time_consist"] is True:
                camera_consistency_loss_dict = compute_camera_consistency_loss(predictions, batch, **self.camera)             
                camera_consistency_loss = camera_consistency_loss_dict["loss_camera_consist"] * self.camera["weight_t_consist"]   
                total_loss = total_loss + camera_consistency_loss
                loss_dict.update(camera_consistency_loss_dict)                
...
def compute_camera_decoder_loss(
    ...
    weight_focal=0,       # weight for focal length loss
    **kwargs
):
    # List of predicted pose encodings per stage
    # predictions["mv_enc_list"],predictions["seq_enc_list"], predictions["mv_similarity"], predictions["seq_similarity"]
    ...
    n_stages = len(pred_dict['mv_enc_list'])
    ...
    B, S, _, _ =  gt_extrinsics.size()
    N_local = 2
    multiview_gt_extrinsics = gt_extrinsics.view(B,N_local,S//N_local,3,4)[:,:,0,:,:]
    multiview_gt_intrinsics = gt_intrinsics.view(B,N_local,S//N_local,3,3)[:,:,0,:,:]
    
    seq_gt_extrinsics = gt_extrinsics.view(B,N_local,S//N_local,3,4)[:,0,:,:,:]
    seq_gt_intrinsics = gt_intrinsics.view(B,N_local,S//N_local,3,3)[:,0,:,:,:]
    
    gt_multiview_encoding = extri_intri_to_pose_encoding(
        multiview_gt_extrinsics, multiview_gt_intrinsics, image_hw, pose_encoding_type=pose_encoding_type
    )
    gt_seq_encoding = extri_intri_to_pose_encoding(
        seq_gt_extrinsics, seq_gt_intrinsics, image_hw, pose_encoding_type=pose_encoding_type
    )
    gt_multiview_encoding = gt_multiview_encoding.unsqueeze(2).reshape(B,-1,9)
    gt_seq_encoding = gt_seq_encoding.unsqueeze(1).reshape(B,-1,9)
    
    # Initialize loss accumulators for translation, rotation, focal length
    total_loss_T = total_loss_R = total_loss_FL = total_loss_T_seq = total_loss_R_seq = 0
    ...
    for stage_idx in range(n_stages):
        ...
        pred_pose_stage = pred_dict["mv_enc_list"][stage_idx]
        ...
        if valid_frame_mask.sum() == 0:
            ...
            loss_T_stage, loss_R_stage, loss_FL_stage = camera_loss_single(
                ...
                gt_multiview_encoding[valid_frame_mask].clone(),
                loss_type=loss_type
            )
        ...
        # *********************************
        pred_seq_stage = pred_dict["seq_enc_list"][stage_idx]
        if valid_frame_mask.sum() == 0:
            # If no valid frames, set losses to zero to avoid gradient issues
            loss_T_stage_seq = (pred_seq_stage * 0).mean()
            loss_R_stage_seq = (pred_seq_stage * 0).mean()
        else:
            # Only consider valid frames for loss computation
            loss_T_stage_seq, loss_R_stage_seq, _ = camera_loss_single(
                pred_seq_stage[valid_frame_mask].clone(),
                gt_seq_encoding[valid_frame_mask].clone(),
                loss_type=loss_type
            )

        # Accumulate weighted losses across stages
        total_loss_T_seq += loss_T_stage_seq * stage_weight
        total_loss_R_seq += loss_R_stage_seq * stage_weight

    # print("total_loss_T_seq",total_loss_T_seq)
    # assert 1==0
    ...
    avg_loss_T_seq = total_loss_T_seq / n_stages
    avg_loss_R_seq = total_loss_R_seq / n_stages
    # Compute total weighted camera loss
    # consist loss
    # C,K,S,_ = pred_dict["mv_similarity"][-1].size()
    # mv_var = pred_dict["mv_similarity"][-1].view(C,K,S*S).var(dim=-1).mean()
    # seq_var = pred_dict["seq_similarity"][-1].view(S,K,C*C).var(dim=-1).mean()
    
    total_camera_loss = (
        avg_loss_T * weight_trans +
        avg_loss_R * weight_rot +
        avg_loss_FL * weight_focal+
        avg_loss_T_seq * weight_trans+
        avg_loss_R_seq * weight_rot
    )
    ...
    return {
        ...
        "loss_R": avg_loss_R,
        "loss_FL": avg_loss_FL,
        "loss_T_seq": avg_loss_T_seq,
        "loss_R_seq" :avg_loss_R_seq,
        # "loss_var": mv_var+seq_var
    }
...
def compute_camera_consistency_loss(
    pred_dict,              # predictions dict, contains pose encodings
    batch_data,             # ground truth and mask batch dict
    loss_type="l1",         # "l1" or "l2" loss
    gamma=0.6,              # temporal decay weight for multi-stage training
    ...
    **kwargs
    ...
    for stage_idx in range(n_stages):
        ...
        loss_T_stage, loss_R_stage = camera_loss_consistency_single(
    ...
    # Return loss dictionary with individual components
    return {
        "loss_camera_consist": total_camera_loss,
        "loss_T_consist": avg_loss_T,
        "loss_R_consist": avg_loss_R,
    }
    
def camera_loss_consistency_single(pred_pose_enc):
    # B D

    B, N, D = pred_pose_enc.size()
    pred_pose,_ = pose_encoding_to_extri_intri(pred_pose_enc,build_intrinsics=False)
    pred_pose = pred_pose.view(-1,3,4)
    pred_pose = closed_form_inverse_se3(pred_pose)
    R = pred_pose[...,:3,:3]
    q = mat_to_quat(R)
    T = pred_pose[...,:3,3]

    pred_pose_enc = torch.cat([T,q],dim=-1)
    N_cam = 2
    N_seq = N//N_cam 

    pred_pose_enc = pred_pose_enc.view(N_cam,N_seq,-1).contiguous()
    # loss by every time
    i1_, i2_ = torch.combinations(torch.arange(N_cam), 2, with_replacement=False).unbind(-1)
    i1, i2 = [(i[None] + torch.arange(B)[:, None] * N_cam).reshape(-1) for i in [i1_, i2_]]

    pred_pose_enc_1 = pred_pose_enc[i1,:,:]
    pred_pose_enc_2 = pred_pose_enc[i2,:,:]
    compare_extric_enc = compare_extric(pred_pose_enc_1,pred_pose_enc_2)

    i1_, i2_ = torch.combinations(torch.arange(N_seq), 2, with_replacement=False).unbind(-1)
    i1, i2 = [(i[None] + torch.arange(B)[:, None] * N_seq).reshape(-1) for i in [i1_, i2_]]
    compare_extric_enc_1 = compare_extric_enc[:,i1,...]
    compare_extric_enc_2 = compare_extric_enc[:,i2,...]
    
    loss_T = (compare_extric_enc_1[..., :3] - compare_extric_enc_2[..., :3]).abs()
    loss_R = (compare_extric_enc_1[..., 3:7] - compare_extric_enc_2[..., 3:7]).abs()

    loss_T = check_and_fix_inf_nan(loss_T, "loss_T_consist")
    loss_R = check_and_fix_inf_nan(loss_R, "loss_R_consist")
    loss_T = loss_T.clamp(max=100).mean()
    loss_R = loss_R.mean()
    
    return loss_T, loss_R

def compare_extric(pose_enc_1,pose_enc_2):
    T_1 = pose_enc_1[..., :3]
    quat_1 = pose_enc_1[..., 3:7]
    
    T_2 = pose_enc_2[..., :3]
    quat_2 = pose_enc_2[..., 3:7]
    
    delta_T = (T_1-T_2)
    delta_q = compute_quad_delta(quat_1,quat_2)
    return torch.cat([delta_T,delta_q],dim=-1)
    
def compute_quad_delta(quat_1,quat_2):
    # q1->q2
    w1,x1,y1,z1 = quat_1[...,0],-quat_1[...,1],-quat_1[...,2],-quat_1[...,3]
    w2,x2,y2,z2 = quat_2[...,0],quat_2[...,1],quat_2[...,2],quat_2[...,3]
