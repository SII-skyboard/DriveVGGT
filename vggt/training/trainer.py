...
class Trainer:
    ...
    def run_train(self):
        ...
        while self.epoch < self.max_epochs:
            print(self.epoch,"self.max_epochs",self.max_epochs)
    ...
    def train_epoch(self, train_loader):      
        ...
        progress = ProgressMeter(
            ...
            num_batches=self.limit_train_batches,
            meters=[
                batch_time,
                data_time,
                mem,
    ...
    def _process_batch(self, batch: Mapping):
        ...
        normalized_extrinsics, normalized_cam_points, normalized_world_points, normalized_depths, normalized_scale= \
        ...
        batch["scale"] = normalized_scale

        # # print(batch['extrinsics'][...,3])
        # point_masks = torch.where(batch["point_masks"]==True,0.85,0.3)


        # pred_dict = \
        # {
        #     "images": torch.Tensor.numpy(batch['images'][0]) ,
        #     "world_points": torch.Tensor.numpy(batch['world_points'][0]),
        #     "world_points_conf": torch.Tensor.numpy(point_masks[0]),
        #     "depth": torch.Tensor.numpy(batch['depths'][0].unsqueeze(-1)),
        #     "depth_conf": torch.Tensor.numpy(point_masks[0]),
        #     "extrinsic": torch.Tensor.numpy(batch['extrinsics'][0]),
        #     "intrinsic": torch.Tensor.numpy(batch['intrinsics'][0]),
        # }
        # import sys
        # sys.path.append("/inspire/hdd/global_user/chenxinyan-240108120066/liuyanhao/vggt")
        # from demo_viser import viser_wrapper
        # print(batch['extrinsics'][0][...,:3,3])
    ...
    def _step(self, batch, model: nn.Module, phase: str, loss_meters: dict):
        ...
        y_hat = model(images=batch["images"], others=images_info)
